...
The three integers immediately after the opcode tell you these three position
...
def compute_day():
  ...
  print('value at position zero (part 1): ', part_1_result)

  noun_and_verb = find_params_that_produce_output(19690720)
  part_2_result = noun_and_verb[0] * 100 + noun_and_verb[1]
  print('100 * noun + verb (part 2): ', part_2_result)
  return [part_1_result, part_2_result]

def find_params_that_produce_output(output):
  noun = 0
  verb = 0
  while noun < 100:
    while verb < 100:
      int_code_result = run_int_code(noun, verb)
      if int_code_result == output:
        return (noun, verb)
      verb += 1
    verb = 0
    noun += 1
...
def run_int_code(noun, verb):
  codes = [int(j) for j in int_code.split(',')]
  i = 0
  codes[1] = noun
  codes[2] = verb
  while i < len(codes):
    ...
    if current_code == 1:
      input_1 = codes[codes[i + 1]]
      input_2 = codes[codes[i + 2]]
...
int_code = '1,0,0,3,1,1,2,3,1,3,4,3,1,5,0,3,2,1,10,19,1,6,19,23,2,23,6,27,2,6,27,31,2,13,31,35,1,10,35,39,2,39,13,43,1,43,13,47,1,6,47,51,1,10,51,55,2,55,6,59,1,5,59,63,2,9,63,67,1,6,67,71,2,9,71,75,1,6,75,79,2,79,13,83,1,83,10,87,1,13,87,91,1,91,10,95,2,9,95,99,1,5,99,103,2,10,103,107,1,107,2,111,1,111,5,0,99,2,14,0,0'